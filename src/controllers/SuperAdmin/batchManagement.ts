...
const batchmanagement = async (req: Request, res: Response) => {
  try {
    const { batchId, endDate, startDate, BatchName, include_saturdays } =
      req.body;
    if (!batchId) {
      ...
      return res
        .status(200)
        .json("No Start date or End date or batch name is given ");
    } else {
      ...
      if (batch === null) {
        ...
        return res
          .status(200)
          .json("Batch details has been updated successfully ");
      }
    }
  } catch (err) {
