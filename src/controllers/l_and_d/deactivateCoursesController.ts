...
const deactivateCourseController = async (req: Request, res: Response) => {
  try {
    ...
    const batchesAssignedToThisCourse =
      await findBatchesAssignedToACourseByCourseSetIdServices(course_set_id);
    if (batchesAssignedToThisCourse) {
      for (const obj of batchesAssignedToThisCourse) {
        const endDateOfTheBatch = await findEndDateOfTheBatchByBatchIdServices(
          obj.batch_id
        );
        if (endDateOfTheBatch) {
          ...
          if (endDateOfTheBatch >= current_date)
            return res
              ...
              .json({
                error:
                  "Cannot delete this course since it is already assigned to an Active Batch.",
              });
        }
        console.log("Hi-------");
      }
    ...
    console.log("Hi-------");
    const deactivateCoursesStatus = await deactivateCoursesInTheListServices(
      course_set_id
    );
