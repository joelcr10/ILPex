...
import findAssessmentService from "../../services/l_and_d_Services/updateAssessment/findAssessmentService";
...
const updateAssessments = async(req:Request,res:Response)=>{
    try{
        ...
        const assessment_found = await findAssessmentService(assessment_id);
        if(assessment_found){
            const batch = parseInt(batch_id);
            if(assessment_found.batch_id===batch){
            ...
            return res.status(404).json({error : "No such assignment is found"});
        }
    }
    catch(err:any){
        console.log(err);
