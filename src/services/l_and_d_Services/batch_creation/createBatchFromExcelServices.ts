...
const createBatchFromExcelServices = async (
  req: Request,
  res: Response,
  inputPath: string,
  batch_name: string,
  ...
  const jsonBatchData: ExcelRow[] =
    XLSX.utils.sheet_to_json<ExcelRow>(batchSheet);
  ...
  for (const row of jsonBatchData) {
    ...
    if (findBatch) {
      ...
      const userCreation = await createUserServices(
        Name,
        Role,
        Email,
        Percipio_Email,
      ...
      if (newUser_id && findBatch.batch_id) {
        const traineeCreation = await createTraineeServices(
          newUser_id,
          findBatch.batch_id,
          userID
        );
      ...
      const batchCreation = await createBatchServices(
        batch_name,
        start_date,
        end_date,
        userID,
      ...
      if (batchCreation) {
        ...
        if (courseAllocationCounter === 0) {
          const courseBatchAllocation = await courseBatchAllocationServices(
            batchCreation.batch_id,
  ...
  if (traineesInTheBatchCounter === 0) {
    const deleteCreatedBatch = await deleteCreatedBatchByBatchId(
      batch_id_global,
      course_batch_allocation_id_global
    );
    return {
      status: 400,
      error:
        "Could not create Batch because all of the trainee details Are duplicate",
    };
  }
  return {
    status: 200,
